...
log_file_path = "script_output.log"  # You can customize the file path
...
# Load secrets
def load_secrets(file_path='b2_secrets.json'):
    try:
        ...
        raise ValueError("Secrets file is not a valid JSON.")
...
def generate_user_agent():
    ...
    locales = ['en_US']
    instagram_versions = ['105.0.0.11.118', '107.0.0.12.121', '108.0.0.14.124', '110.0.0.16.119', '111.0.0.25.121', '113.0.0.39.122']
    ...
    while attempt < max_retries:
        try:
            ...
            if attempt >= max_retries:
                ...
                return None
            time.sleep(delay)  # Wait for 100 milliseconds before retrying
...
def process_username_full(username, config):
    proxy_file = './script_inputs/datacenter_proxy.txt'
    file_name = f"./jsons/{username}'s_JSON_QUERY.json"
    scraped_usernames_path = './scraped_usernames.txt'
    ...
    # Check if the username exists in 'scraped_usernames.txt'
    if os.path.exists(scraped_usernames_path):
        with open(scraped_usernames_path, 'r') as f:
            if any(line.strip() == username for line in f):
                logging.info(f"Username '{username}' already recorded in 'scraped_usernames.txt'. Skipping...")
                return

    res_proxies = config["residential-proxies"]
    datacenter_proxies = config["datacenter-proxies"]

    # Select a random residential proxy from the list
    res_proxy = get_random_proxy(res_proxies)
